...
- **Development re-opened at `0.1.2.dev0`** (card REL-12). `0.1.1` is published and its tag is
  used, so `main` could not keep declaring it: while it did, every build made from a checkout
  named a release it was not, and a second `v0.1.1` tag on a later commit would have passed the
  release gate and fired the publish leg at a version the index already holds. `pyproject.toml`,
  `gebra.__version__` and `uv.lock` now declare `0.1.2.dev0` together — PEP 440's dev segment off
  the next patch, the form PD-036 ruled — so `tools/release_gate.py --tag v0.1.1` refuses with a
  ...
  `0.1.1` moves: `pip install gebra` installs it exactly as before, the PyPI badge goes on reading
