...
All five per-validator pages are written, and each goes further on its own property than this
table can: [P-01 `graph-well-formed`](../validators/p01-graph-well-formed.md),
...
[P-08 `determinism-replay`](../validators/p08-determinism-replay.md). Each one says what its
property reads, what its witness and failure records contain field by field, and where its claim
stops.
