...
Four of the per-validator pages are written — [P-01 `graph-well-formed`](../validators/p01-graph-well-formed.md),
...
[P-06 `effect-safety`](../validators/p06-effect-safety.md) — and the last one is still a
placeholder. Where a page is not yet written, the authority on what that property's
