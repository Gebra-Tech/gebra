...
| The artifact it renders verdicts from | **REPORT-FORMAT-SPEC** (final; `report_format` `1.3`) |
...
| List findings (severity, claim class, condition ID, location) and pass witnesses | `gebra verify --format json` | REPORT-FORMAT-SPEC §1 (`report_format` `1.3`) |
